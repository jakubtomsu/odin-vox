...
load_from_data :: proc(
	...
	result.palette = transmute(Pallette)DEFAULT_PALETTE
	...
	num_models := 1
	{
		...
		if next_id == {'P', 'A', 'C', 'K'} {
			...
			pack := read_data(&data, Pack_Chunk) or_return

			num_models = int(pack.num_models)
		}
	}

	result.models = make([]Model, num_models, allocator)
	model_index := 0
	...
	for len(data) > 0 {
		...
		switch chunk.id {
		case {'S', 'I', 'Z', 'E'}:
			size := read_data(&data, Size_Chunk) or_return
			xyzi_chunk := read_data(&data, Chunk) or_return
			assert(xyzi_chunk.id == {'X', 'Y', 'Z', 'I'})
			xyzi := read_data(&data, Xyzi_Chunk) or_return

			model := Model {
				size = {int(size.size.x), int(size.size.y), int(size.size.z)},
				voxels = make([]Voxel, int(xyzi.num_voxels), allocator),
			}

			for i in 0 ..< xyzi.num_voxels {
